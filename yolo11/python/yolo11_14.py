...
# 在类定义中添加摄像头画面尺寸常量
CAMERA_WIDTH = 640  # 摄像头采集画面的宽度
CAMERA_HEIGHT = 480  # 摄像头采集画面的高度
...
class TCMDetectionApp:
    def __init__(self, root, model, platform, args):
        # 定义摄像头分辨率常量
        self.CAMERA_WIDTH = 640  # 摄像头宽度
        self.CAMERA_HEIGHT = 480  # 摄像头高度      
        self.root = root
        self.model = model
        # 仅放大字号（从9调整到11），保持其他属性不变
        default_font = ('nimbus sans l', 14)  # 保持原字体家族，只改大小
        ...
        if self.is_camera_running and self.cap:
            ...
            if ret:
                ...
                if self.video_image is None:
                    ...
                    self.video_canvas.itemconfig(self.video_image, image=self.current_photo)
