...
class TCMDetectionApp:
    ...
    def create_widgets(self):
        ...
        # 设置基础字体（保持Nimbus Sans L但调整大小）
        base_font = ('nimbus sans l', 14)  # 从默认9号放大到10号
        # 设置按钮样式（使用默认字体，字号14）
        style.configure('TButton', font=('nimbus sans l', 14))
        # 配置各部件样式
        style.configure('.', font=base_font)  # 全局默认
        style.configure('TLabel', font=base_font)
        style.configure('TButton', font=base_font)
        style.configure('TEntry', font=base_font)
        style.configure('TFrame', font=base_font)
        # 左侧检测区域
        ...
        # 强制设置最小尺寸（防止窗口太小导致变形）
        self.video_frame.config(width=400, height=480)
        self.video_frame.pack_propagate(False)  # 禁止自动调整尺寸
        ...
        self.status_bar.pack(side=tk.LEFT, padx=5)  # 固定在底部
    ...
    def load_herb_properties(self, file_path):
        ...
        try:
            ...
            print(f"警告: 未找到药材属性文件 {file_path}")
        except Exception as e:
            print(f"加载药材属性文件出错: {str(e)}")
    
        return properties   
    def update_export_button_state(self):
        """更新导出按钮状态"""
        if self.detection_results:  # 如果有检测结果
    ...
    def update_result_text(self):
        """更新结果文本框，显示检测到的药材及其功能性状"""
        self.result_text.config(state=tk.NORMAL)
        self.result_text.delete(1.0, tk.END)
    
        if self.detection_results:
            for result in self.detection_results:
                herb_name = result['class']
                properties = self.herb_properties.get(herb_name, {})
            
                line = f"【{herb_name}】\n"
                line += f"置信度: {result['score']:.2f}\n"
                line += f"位置: {result['box']}\n"
            
                if properties:
                    line += f"功能: {properties.get('功能', '未知')}\n"
                    line += f"主治: {properties.get('用于', '未知')}\n"
                    line += f"性状: {properties.get('形状', '未知')}\n"
                else:
                    line += "未找到该药材的详细信息\n"
