...
class TCMDetectionApp:
    ...
    def create_widgets(self):
        ...
        self.ai_init_btn = ttk.Button(
            ai_init_frame, 
            text="初始化AI模型", 
            command=self.start_ai_initialization,
    ...
    def check_ai_initialization_status(self):
        ...
        if self.is_ai_initializing:
            ...
            if self.llm is not None:
                self.ai_status_label.config(text="AI模型已就绪", foreground="green")
                self.ai_ask_btn.config(state=tk.NORMAL)
                self.status_bar.config(text="AI模型初始化完成，可以开始提问")
                self.append_ai_message("AI助手", "AI模型已初始化，可以开始提问中药材相关问题了！")
            else:
